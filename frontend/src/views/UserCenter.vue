<template>
  <div class="max-w-6xl mx-auto px-4 py-8">
    <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <div class="text-2xl font-bold text-gray-900">个人中心</div>
        ...
        <el-tab-pane label="我的订单" name="orders">
          <div class="py-4 space-y-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div class="text-sm text-gray-500">支持状态筛选、详情查看、支付、取消、评价与售后。</div>
              <div class="flex items-center gap-3">
                <el-select v-model="orderStatus" placeholder="订单状态" style="width: 180px" @change="fetchOrders">
            ...
            <el-alert v-if="!hasToken" type="warning" show-icon title="未登录" description="请先登录后查看订单。" />
            ...
            >
              ...
              <el-table-column label="操作" width="260" fixed="right">
                <template #default="{ row }">
                  ...
                  <el-button v-if="row.status === 'pending'" link type="success" @click="openPay(row._id)">支付</el-button>
                  <el-button v-if="canCancel(row.status)" link type="warning" @click="cancelOrder(row._id)">取消</el-button>
                  <el-button v-if="row.status === 'delivered'" link type="primary" @click="openReview(row._id)">评价</el-button>
                  <el-button v-if="canAfterSale(row)" link type="danger" @click="openAfterSale(row._id)">售后</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
        ...
        <el-tab-pane label="账户安全" name="security">
          <div class="py-6 max-w-xl">
            ...
            <div v-else class="space-y-4">
              <div>
                <div class="text-base font-semibold text-gray-900">修改密码</div>
                <div class="text-sm text-gray-500 mt-1">修改成功后会自动退出，需要重新登录。</div>
              </div>
              ...
              <el-form ref="pwdFormRef" :model="pwdForm" :rules="pwdRules" label-width="96px" status-icon>
                ...
                <el-form-item>
                  <div class="flex items-center gap-3">
                    <el-button type="primary" :loading="pwdSubmitting" @click="submitChangePassword">保存</el-button>
  ...
  <el-drawer v-model="detailOpen" title="订单详情" size="520px">
    ...
    <div v-else-if="orderDetail" class="space-y-4">
      <div class="text-sm text-gray-500">订单号：{{ orderDetail._id }}</div>
      <div class="flex items-center justify-between">
        <div class="text-lg font-semibold text-gray-900">金额：¥{{ Number(orderDetail.total_amount ?? 0).toFixed(2) }}</div>
        <el-tag :type="statusTagType(orderDetail.status)">{{ statusLabel(orderDetail.status) }}</el-tag>
      </div>
      <div class="text-sm text-gray-600">收货地址：{{ orderDetail.shipping_address || '-' }}</div>
      <div class="text-sm text-gray-600">支付：{{ paymentText }}</div>
      <div v-if="orderDetail.cancel_request" class="text-sm text-gray-600">取消原因：{{ orderDetail.cancel_request.reason }}</div>
      <div v-if="orderDetail.after_sale" class="text-sm text-gray-600">
        售后：{{ orderDetail.after_sale.type }} / {{ orderDetail.after_sale.status }} / {{ orderDetail.after_sale.reason }}
      </div>
      <div v-if="orderDetail.review" class="text-sm text-gray-600">评价：{{ orderDetail.review.rating }} 星 / {{ orderDetail.review.content }}</div>
      <div class="text-base font-semibold text-gray-900 mt-4">商品项</div>
  ...
  <el-dialog v-model="payOpen" title="订单支付" width="420px">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="payLoading" :disabled="!payMethod" @click="submitPay">确认支付</el-button>
    </template>
  </el-dialog>
...
const activeTab = ref<'orders' | 'profile' | 'security' | 'address'>('orders')
...
async function fetchProfile() {
  ...
  try {
    ...
    const data = unwrap<{ username?: string; phone?: string }>(resp)
